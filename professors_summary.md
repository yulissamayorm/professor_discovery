...
| 3 | [Daniela Galatro](#daniela-galatro) | University of Toronto (Department of Mechanical and Industrial Engineering) | 2 (of 3 expected) | Tabular accelerated-aging measurements of Li-ion cells (capacity vs. time across T, SOC, C-rate, DOD, chemistry); semi-empirical regression (Article 1 — Dakin / Arrhenius / quadratic in stress factors) + 3-parameter non-homogeneous Gamma process + Monte Carlo pack rollup (Article 2) | 6.8 / 10 (provisional) | Provisional — Articles 1 and 2 processed (BOTH FIRST-AUTHORED, corresponding-author papers — strongest lab signal); awaiting personal-interest scores and Article 3 |
...
- **Common thread across their work (based on Articles 1 and 2):** Statistical and stochastic-process modelling of lithium-ion battery aging in EV-relevant regimes. Article 1 dissects why the field-standard averaged-α regression (Mathieu et al. 2017) breaks across chemistries and stress regimes; Article 2 proposes a 3-parameter non-homogeneous Gamma process replacement that captures cell-to-cell variation natively and rolls up to module/pack level via Monte Carlo. **Both papers are tabular-only, parametric, and contain zero learned ML** — the lab's expertise is statistical reliability modelling rather than ML, but the data and problem framing port cleanly to ML.
- **Dominant data modality:** Tabular accelerated-aging measurements — small datasets (tens to hundreds of rows), assembled from cited public sources rather than lab-generated raw data.
- **Biggest obstacle for me to replicate their work publicly:** Data assembly — the underlying datasets (Keil 2016, Wikner 2017, Baumhöfer 2014, MOBICUS) are public but live in source-paper tables, theses, and supplementary materials rather than as unified downloadable CSVs. **No code release** for either Galatro paper.
- **Skills I would gain by working with them:** Battery aging / RUL / SOH prediction with proper uncertainty quantification (a recognised industrial ML problem); accelerated-test experimental design + sample-size analysis; multi-factor regression with full goodness-of-fit diagnostics; Gamma stochastic processes + Monte Carlo pack rollup; Arrhenius-style acceleration factors as physics-informed inputs; cross-chemistry transfer; pack-level architecture-aware modelling (parallel sum / series min compositions).

### Professor-level rollup

- **Article 1 overall:** 6.6 (provisional)
- **Article 2 overall:** 6.9 (provisional)
- **Article 3:** Not yet processed.
- **Professor overall** = (6.6 + 6.9) / 2 = 13.5 / 2 = **6.8 / 10** (provisional — both articles awaiting personal-interest scores; will be recomputed once Article 3 is processed)
...
- Personal interest score for Article 1 not yet provided by student.
- Personal interest score for Article 2 not yet provided by student.
- Article 3 (`3.Battery Health Diagnosis Approach_TSpace.pdf`) is pending — available in `C:\Users\Yulissa\OneDrive\Escritorio\PDFs Professors\Galatro\`.
- Dataset assembly is the highest-friction step for portfolio projects based on these articles — confirm digitisation strategy (or substitution with NASA / CALCE / Severson) before committing to it.
