...
| 7 | [Frank Gu](#frank-gu) | University of Toronto (Department of Chemical Engineering and Applied Chemistry; joint with University of Waterloo Department of Chemical Engineering on Article 1) | 1 (of N expected) | Heterogeneous photocatalyst characterisation (TEM micrographs of noble-metal-on-TiO₂ nanoparticles, BET surface area, DLS hydrodynamic diameter) + tabular catalyst descriptors (work function, particle size, support band gap) → Se-product selectivity regression; ML reframe routes via Open Catalyst Project (OC20 / OC22) for surface-adsorption GNN baselines | 5.3 / 10 (provisional) | Provisional — Article 1 only (CORRESPONDING-AUTHOR senior-PI signal); awaiting personal-interest score and confirmation of how many articles to process |
...
- **Articles 2 / 3:** Not yet processed — pending student direction on whether more articles will be added.
- **Professor overall** = 5.3 / 1 = **5.3 / 10** (provisional, single-article — will be recomputed once additional articles are added)

### Summary for this professor

- **Common thread across their work (based on Article 1 only):** Heterogeneous nanoscale photocatalysts for water-treatment chemistry — synthesis of metal-decorated semiconductor nanoparticles, mechanistic study of charge-carrier dynamics (Schottky junctions, Fermi-level equilibration, quantised double-layer charging), and selectivity engineering for environmental contaminants. **One-paper inference; treat as tentative until at least one more article is processed.**
- **Dominant data modality:** Heterogeneous-photocatalyst characterisation data (TEM, BET, DLS, EELS) + tabular reaction kinetics / selectivity time-series, with no released raw dataset; the recruiter-legible ML reframe pulls the modality up to atomic-structure GNN (Open Catalyst Project) at a substantial tooling cost.
- **Biggest obstacle for me to replicate their work publicly:** **Two separate obstacles, both substantial.** First, no public dataset exists for selenate photocatalysis — any portfolio project must either accept hand-digitised small data from this paper + cited literature, or substitute a related but non-selenate benchmark from Open Catalyst Project / Catalysis Hub. Second, the strongest portfolio framing (OC20-style GNN) imposes the heaviest new-tooling lift in the survey (ASE + FAIR-Chem + atomic-structure GNNs on top of `pymatgen`), which is a substantial jump from the student's current sklearn / PyTorch baseline.
- **Skills I would gain by working with them:** Heterogeneous-catalyst synthesis intuition (deposition–precipitation, photodeposition, calcination); semiconductor band-edge reasoning + Schottky-junction physics + Fermi-level equilibration on nanoparticles; UV-photocatalysis reactor design + actinometry; HG-ICP-OES + DHE-fluorimetry analytical chemistry; potentially (if pursued) Open Catalyst Project / FAIR-Chem GNN tooling for catalyst-screening ML. **All of these are useful but the catalyst-synthesis and analytical-chemistry skills are wet-lab — they will not directly produce GitHub commits.**

### Open items / notes

- Lab webpage / Google Scholar URL for Frank Gu not yet confirmed — verify before finalising. Note: dual affiliation (Waterloo + U of T) on this paper means his current primary appointment may need confirmation; a 2025 / 2026 first-/corresponding-author paper from the U of T-only side would be the strongest signal.
- Personal interest score for Article 1 not yet provided by student.
- **Important framing-sensitive caveat:** the 5.3 score is dominated by the tooling cost of the highest-impact reframe (OC20 GNN, score 4 on tooling). If the student is willing to accept a less recruiter-flashy but lower-tooling-cost framing (Path 1 tabular regression in the portfolio-angle section above), the *effective* score for that path is closer to 6.0 — Tooling jumps from 4 → 7, raising the overall to (5 + 6 + 8 + 7 + 5 + 3 + 7) / 7 = 41 / 7 ≈ 5.9. **The score reported is for the recruiter-impact path, not the easiest path.** Discuss preferences before locking it in.
- **Lab-direction uncertainty:** Article 1 is a water-treatment / heterogeneous-catalysis paper. Frank Gu's broader publication record almost certainly includes biomedical / drug-delivery nanoparticle work (a different research thrust at U of T Chemical Engineering), which would substantially change the modality and dataset profile and likely raise the tooling-alignment score. **Strongly recommend processing at least one additional article — ideally from a different research thrust — before treating the 5.3 rollup as a stable advisor signal.**
- **Three viable portfolio paths** at very different tooling costs are scoped in the Article 1 portfolio-angle section above (tabular regression / TEM-image segmentation / OC20-style GNN). The student should pick a path before the project starts, since the choice changes the overall rubric scores by ~1 point and changes the weeks-to-MVP question (which this rubric does not score, per `Claude.md`).
- Confirm whether Frank Gu currently supervises master's theses at U of T (not just PhDs / postdocs) — the joint Waterloo / U of T affiliation on this paper means the supervisory pathway may flow through one or the other institution.
