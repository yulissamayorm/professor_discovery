...
**Affiliation:** University of Toronto, Department of Chemical Engineering and Applied Chemistry.

**Lab focus (one sentence):** **Inferred** from Article 1 + her co-authorship of multiple metagenomics / bioremediation papers cited in this Review (KB-1, ANAS, WBC-2, ACT-3 enrichment cultures): environmental engineering / anaerobic microbiology with emphasis on **organohalide-respiring bacteria** for groundwater bioremediation — i.e., microbial communities that detoxify chlorinated solvents (PCE, TCE, vinyl chloride, chlorinated ethanes, etc.) by using them as terminal electron acceptors. **Verify against the lab's current Toronto-era publications before treating this as authoritative.**

**Lab webpage / Scholar:** Not yet provided — needs to be sourced.

### Article 1: Overview of organohalide-respiring bacteria and a proposal for a classification system for reductive dehalogenases

- **Full citation:** Hug, L. A., Maphosa, F., Leys, D., Löffler, F. E., Smidt, H., **Edwards, E. A.**, & Adrian, L. (2013). Overview of organohalide-respiring bacteria and a proposal for a classification system for reductive dehalogenases. *Philosophical Transactions of the Royal Society B*, 368(1616), 20120322. DOI: 10.1098/rstb.2012.0322
- **Authorship caveat:** Edwards is the **6th of 7 authors** (middle author) on this **Review/Perspective** article. The corresponding author is Hauke Smidt (Wageningen). First author Laura A. Hug was Edwards's PhD student at U of T. **Treat this as evidence of an ongoing collaborative research direction (Edwards's group at Toronto contributed enrichment-culture metagenomics — KB-1, ACT-3, WBC-2 — that fed into the curated RdhA set) rather than a flagship Edwards-lab methodology paper.**
- **Problem in plain English:** Anaerobic bacteria called **organohalide-respiring bacteria (OHRB)** can break carbon–halogen bonds in chlorinated pollutants (industrial degreasers like tetrachloroethene/PCE, dry-cleaning fluids, agricultural chemicals) and use them as the equivalent of oxygen for energy. The enzymes responsible — **reductive dehalogenases (RDases)** — are encoded by genes called *rdhA*. Each OHRB genome can carry up to 36 different *rdhA* genes, and >600 reductive-dehalogenase sequences exist in NCBI as of 2012. The field has a naming-and-classification crisis: there's no consistent way to name these enzymes, and **sequence similarity does NOT reliably predict substrate specificity** (two enzymes 95% identical can dechlorinate completely different substrates; two enzymes that both dechlorinate PCE can be at opposite ends of the phylogenetic tree). The paper proposes a phylogenetic-orthologue-based classification system (the **RD_OG framework**) by curating 264 RdhA proteins from 44 organisms and clustering them with a 90%-pairwise-identity threshold into 46 orthologue groups.
- **ML task type (REFRAMED — paper itself contains no ML):** The paper is phylogenetics + sequence analysis. The natural ML reframing of the paper's *open problem* — "sequence similarity does not predict substrate specificity" — is **protein function prediction**: a multi-label classification task that takes an RdhA amino-acid sequence as input and predicts the chlorinated substrates the enzyme can dechlorinate (PCE / TCE / cis-DCE / VC / 1,2-DCA / chlorophenols / chlorobenzenes / chloroethanes / chloroform / etc.).
- **Input representation:**
  - Modality: **Protein amino-acid sequences** (variable length, ~300–600 residues for full-length RdhA proteins).
  - Shape/size: 264 curated full-length RdhA proteins from 44 organisms (paper) — this is the labelled training set; many more reductive-dehalogenase entries exist in UniProt for unlabelled pretraining or self-supervised fine-tuning.
  - Preprocessing in the paper: MUSCLE alignment + manual masking of ambiguously aligned positions; final alignment 831 amino-acid positions. Phylogenetic trees built with RAxML (max-likelihood) and MR.BAYES (Bayesian inference). For ML reframing, you'd skip the alignment and feed raw sequences to a protein language model.
- **Output representation:**
  - What the model predicts: **Multi-label substrate specificity** — for each known chlorinated substrate (e.g., PCE, TCE, cis-DCE, VC, 1,2-DCA, 2,4,6-trichlorophenol, 1,2,3-trichlorobenzene, chloroform, etc.), a binary "this RdhA dechlorinates it / does not." A simpler v0 task: predict the orthologue group (1 of 46 RD_OGs) from sequence — this is well-labelled (176 of 264 proteins are placed into groups in the paper) and directly testable.
  - Units / label space: Multi-label binary vector over ~10–15 substrate categories (very sparse — only **15 of the 264 RdhA proteins** in the paper have *any* characterized substrate — see numbered enzymes 1–15 in Figure 2). Or: 46-class single-label classification (more abundant labels for the orthologue-group prediction task).
- **Model architecture (paper has none — this is a suggestion):** ESM-2 (8M or 35M parameters, free-Colab compatible) protein language model → mean-pooled per-protein embedding → small MLP / logistic-regression head for substrate or orthologue-group prediction.
- **Dataset used in the paper:**
  - Name: Curated set of **264 RdhA proteins** from 44 organisms (electronic supplementary material, table S1 of the paper).
  - Size: 264 protein sequences with metadata (organism, gene name, accession, orthologue group assignment); ~15 with biochemically characterized substrate specificity; ~50–100 more with putative substrate assignments via transcriptional / proteomic studies.
  - Source: Mined from finished genome sequences in IMG-M and NCBI, plus metagenomes of dechlorinating consortia (KB-1, ACT-3, WL — several of which are Edwards-lab cultures).
  - Public? **Yes — sequences are accessible via NCBI / UniProt by accession.** Supplementary table S1 has the accession list.
  - Link: Supplementary materials at http://dx.doi.org/10.1098/rstb.2012.0322 . The paper also references a **Google Drive folder** (docs.google.com/folder/d/0BwCzK8wzlz8ON1o2Z3FTbHFPYXc/edit) for the curated dataset and RD_OG spreadsheet; **this 2013-vintage Google Drive link is almost certainly dead by 2026 — verify before relying on it.**
- **Reported headline metric:** No predictive metric — the paper's "result" is descriptive: 264 RdhAs cluster into 46 orthologue groups (90% PID threshold, >90% bootstrap support) containing 176 proteins. Key qualitative finding: **sequence similarity ≠ substrate specificity** (e.g., DcrA and CfrA share 95% PID but dechlorinate different chloroethanes; the four PCE dehalogenases sit at three distant tree positions).
- **Public alternatives to replicate or approximate this:**
  - **NCBI / UniProt reductive dehalogenase entries** — search "reductive dehalogenase" returns thousands of entries — UniProt is one-line `requests.get` per accession or bulk download — **easy / medium** — https://www.uniprot.org/ — **close** match for the substrate-prediction reframing.
  - **TIGRFAM TIGR02486 + PFAM PF13486** (reductive dehalogenase protein families) — these are family definitions with member lists — **medium** access via InterPro — https://www.ebi.ac.uk/interpro/ — **close** match.
  - **CAFA challenge data (Critical Assessment of Functional Annotation)** — large public benchmark for protein function prediction (GO term assignment from sequence) — **easy** via the CAFA website — **analogy** match (general protein-function prediction; not RdhA-specific but provides the standard ML task formulation, evaluation metrics, and pretrained baselines).
  - **ESM-2 / ProtBERT pretrained models** — these are not datasets but the standard ML "tooling" for protein-sequence work — **easy** via HuggingFace — match: this is what you'd use as a feature extractor on top of any of the three datasets above.
- **Data access difficulty (for the paper's own dataset):** **Medium** — the 264 sequences are listed by accession in the supplementary table; you'd write a BioPython script that loops over accessions and fetches sequences from NCBI Entrez. The substrate-specificity labels for the 15 characterized RdhAs would have to be hand-extracted from Figure 2 / Table S3 of the paper.
- **Tooling I would need to learn:** **BioPython** (sequence I/O, NCBI Entrez fetching), **HuggingFace `transformers` + ESM-2** (protein-language-model embeddings — significantly different from text/image transformers in tokenization, but the inference API is straightforward), and standard scikit-learn (logistic regression, multi-label classification metrics like macro-F1, precision-at-k). **Optional but recommended:** a protein-alignment tool (MUSCLE or MAFFT) to compare ML-based clustering against the paper's phylogeny-based clustering for a "your model rediscovers the orthologue groups" framing.
- **Portfolio angle — what a good GitHub project based on this would look like:** **"Protein-language-model classifier for reductive-dehalogenase substrate specificity."** The project would (i) fetch the 264 RdhA sequences from NCBI using the accessions in supplementary table S1, (ii) embed each protein with ESM-2 (8M-parameter model on free Colab), (iii) train a logistic-regression / small-MLP classifier on the 15 substrate-labelled enzymes (with cross-validation given the tiny label set) and a separate 46-way classifier on the orthologue-group labels (the well-labelled task), (iv) use the trained model to predict substrate specificity for the ~250 unlabelled RdhAs and flag predictions as candidates for biochemical follow-up, (v) provide a notebook that reproduces the paper's qualitative finding ("sequence similarity ≠ substrate specificity") quantitatively — e.g., correlation of pairwise sequence identity vs. predicted-substrate Jaccard distance. README headline: *"A reproducible ESM-2-based pipeline for classifying organohalide-respiring enzymes — recovers 80%+ of the manually-curated RD_OG orthologue groups from sequence alone, and ranks unlabelled candidates for substrate-specificity follow-up."* This sits squarely in the protein-function-prediction subfield (legible to ML recruiters who know CAFA / ESM) while being grounded in a real environmental-engineering problem (groundwater bioremediation).

#### Ranking for Article 1

| Variable | Score (1–10) | One-line justification |
|---|---|---|
| Data availability | 6 | The 264-sequence curated set is listed by accession in supplementary table S1 and fetchable from NCBI/UniProt; the 15-enzyme substrate-specificity label set is small but explicit in Figure 2; UniProt has many more reductive-dehalogenase entries for unlabelled pretraining — overall a **loose-to-close** match for the protein-function-prediction reframing. |
| Data access ease | 5 | Sequences are accessible via NCBI accession but require a BioPython script that loops over the supplementary table; the Google Drive link in the paper for the RD_OG spreadsheet is from 2013 and likely dead — **scripting / scraping required**. |
| Task tractability on laptop / free GPU | 9 | 264 sequences is tiny. ESM-2 (8M or 35M parameters) embeddings can be computed on free Colab in minutes; the downstream classifier trains in seconds on CPU. **Trivially fits free-tier compute.** |
| Tooling alignment with my current skills | 5 | Heavy reliance on **BioPython** (new) and **HuggingFace ESM-2** (new — protein-language-model embeddings differ in tokenization/usage from standard NLP transformers). Standard sklearn classifier on top is in current toolset. **One major new component** (protein LMs). |
| Problem clarity | 4 | The reframing is clear (sequence → substrate prediction), but the labelled set is **tiny and uncertain** — only 15 of 264 RdhAs have biochemically characterized substrates, and many of those have only partial substrate ranges. The 46-orthologue-group task is well-labelled (176 of 264) but is a less compelling ML problem (it largely recovers a sequence-similarity clustering by construction). |
| Reproducibility signals | 4 | **No code released.** Sequence accessions are public. Methods are described in detail (RAxML, MR.BAYES parameters specified). The Google Drive RD_OG spreadsheet is likely dead. **Metric reported but no code or maintained data release.** |
| Portfolio impact | 5 | Protein function prediction is a **hot ML subfield** (CAFA challenges, ESM-2 / AlphaFold-era), but RdhA-specific application is **niche** (environmental microbiology / bioremediation rather than drug discovery). A clean ESM-2-based classifier would be legible to recruiters, but the application domain (chlorinated-solvent groundwater remediation) requires more framing than e.g. drug-target prediction would. **Interesting but not immediately legible.** |
| My personal interest | (ask me) | (provided by student) |

**Overall score computation:**
- Provisional 7-variable mean: `(6 + 5 + 9 + 5 + 4 + 4 + 5) / 7 = 38 / 7 ≈ 5.4` (provisional — personal interest pending).

**Overall score:** **5.4 / 10** (provisional)

**One-sentence verdict:** Yes — a credible ESM-2-based protein-function-prediction project is shippable from this paper's open question (sequence similarity ≠ substrate specificity), but the very small labelled set (15 enzymes), the niche bioremediation application area, and the middle-authorship of Edwards on a Review paper make this a **second-tier portfolio bet** unless additional Edwards-lab first-author / corresponding-author papers materially raise the per-professor score.

### Article 2: Anaerobic digestion of pulp and paper mill wastewater and sludge

- **Full citation:** Meyer, T., & **Edwards, E. A.** (2014). Anaerobic digestion of pulp and paper mill wastewater and sludge. *Water Research*, 65, 321–349. DOI: 10.1016/j.watres.2014.07.022
- **Authorship caveat:** **Edwards is 2nd of 2 (last/senior author)**; Torsten Meyer (her postdoc/research associate at U of T's Department of Chemical Engineering and Applied Chemistry) is the corresponding author. **This is a markedly stronger authorship signal than Article 1** — a 2-author Review with Edwards as senior author indicates this is a flagship review from her independent Toronto group on the wastewater-engineering side of her research portfolio. The paper sits in a different sub-area than Article 1 (industrial bioenergy / wastewater engineering rather than chlorinated-solvent bioremediation), suggesting the Edwards lab spans **two distinct research directions** at U of T.
- **Problem in plain English:** Pulp and paper mills generate enormous amounts of organic waste in their wastewater (1–10 g L⁻¹ COD, 4–400 million tons COD globally per year) and sludge (0.2–0.6 wet tons sludge per ton of pulp). **Anaerobic digestion** can convert this organic waste to methane biogas (electricity / heat) instead of paying to treat or landfill it — but anaerobic reactors are picky: they only handle some mill streams reliably, and they're sensitive to inhibitors (resin acids, sulfur compounds, organochlorines from bleaching). The Review consolidates ~150 prior studies into structured tables that quantify, for ~30 different mill effluent types: COD concentration, inhibitor concentrations, COD removal rate (15–99%), and specific methane yield (0.0–0.40 m³ kg⁻¹ COD removed). It also reviews ~20 sludge-pretreatment experiments with VS removal rates (21–65%) and methane yields (40–270 mL g⁻¹ VS fed).
- **ML task type (REFRAMED — paper itself contains no ML):** **Tabular regression** — predict COD removal rate (%) and specific methane yield (m³ kg⁻¹ COD removed) from wastewater feedstock composition (COD, inhibitor concentrations, TSS, pulping process, reactor type) and operational parameters (organic loading rate, HRT). Secondary task: **multi-output regression on sludge digestion** — predict VS removal and methane yield from sludge type (primary / biosludge / mixture), pretreatment type (none / thermal / microwave / ultrasound / hydrodynamic / caustic / enzymatic), TS / VS content, and reactor configuration.
- **Input representation:**
  - Modality: **Tabular** — categorical features (wastewater type ≈ 30 classes, reactor type ≈ 5 classes, pretreatment type ≈ 8 classes) + continuous features (COD g L⁻¹, sulfide mg L⁻¹, sulfite mg L⁻¹, sulfate mg L⁻¹, resin acids mg L⁻¹, AOX mg L⁻¹, peroxide mg L⁻¹, TSS mg L⁻¹, organic loading rate kg COD m⁻³ day⁻¹, HRT days).
  - Shape/size: ~50–100 rows extractable from Tables 1, 2, 5 of this paper alone; bibliographic mining of cited references (≈150 refs) could expand to several hundred rows.
  - Preprocessing in the paper: None — the paper *is* the curated dataset (it's a Review). For the ML reframing, manual transcription of Tables 1, 2, 5 to CSV is the preprocessing step.
- **Output representation:**
  - What the model predicts: **(i) COD removal rate (%), 0–100 continuous regression**, and **(ii) specific methane yield (m³ kg⁻¹ COD removed for wastewater, or mL g⁻¹ VS fed for sludge), continuous regression**. Both targets are reported as ranges in the paper (e.g., "50–70% COD removal") — for ML, take the midpoint or fit interval-regression / quantile regression.
  - Units / label space: % for COD removal; m³ kg⁻¹ COD removed for wastewater methane yield; mL g⁻¹ VS fed for sludge methane yield.
- **Model architecture (paper has none — this is a suggestion):** Standard tabular ML stack — gradient-boosted trees (XGBoost / LightGBM / CatBoost) is the default for ~100-row tabular data with mixed categorical + continuous features. SHAP values for feature importance to surface "which inhibitors most reduce predicted methane yield." Optional: a small MLP for comparison; Bayesian-ridge for uncertainty estimates given the small dataset.
- **Dataset used in the paper:**
  - Name: **No standalone dataset** — the paper consolidates literature data into Tables 1 (wastewater × digestibility, ~25 rows), 2 (full-scale / pilot case studies, ~16 rows), 3 (inhibitor thresholds, ~12 rows), 4 (sludge-property comparison, ~12 columns × 3 sludge types), and 5 (sludge digestion experiments × pretreatment, ~30 rows).
  - Size: ~80–100 rows total across the data-bearing tables.
  - Source: Hand-curated review of ~150 published studies (citations in the references list).
  - Public? **Yes — the paper is the dataset**; Tables 1, 2, 5 are publicly readable in the paper PDF. Citations point to the original studies for further data.
  - Link: Paper PDF at https://doi.org/10.1016/j.watres.2014.07.022 (paywalled — accessed via institutional Elsevier/ScienceDirect subscription).
- **Reported headline metric:** No predictive metric — descriptive consolidation. Paper's qualitative headline finding: "**most in-mill streams can be at least a component of the reactor feed in anaerobic treatment**" (contrary to common perception that bleaching effluents and sulfite-rich streams are categorically untreatable), with COD removal >50% achievable for the majority of streams given microbial adaptation and dilution. Methane-yield range: 0.20–0.40 m³ kg⁻¹ COD removed for digestible streams; 0.0–0.40 m³ kg⁻¹ for bleaching effluents (highly variable).
- **Public alternatives to replicate or approximate this:**
  - **Anaerobic Digestion Model No. 1 (ADM1)** — IWA's mechanistic ODE-based simulation framework — implementations available via BSM2 / OpenWARP / PyADM1 — **medium** access (free open-source code on GitHub) — http://iwa-mia.org/benchmark-simulation-models/ — **analogy** match (mechanistic model, not data-driven; useful as a baseline / hybrid component).
  - **AD-DataS / IWA Anaerobic Digestion datasets** — community-shared experimental data from anaerobic-digestion researchers — **hard** access (typically per-paper / per-lab supplementary data, no consolidated benchmark) — **loose** match.
  - **Bibliometric scrape of cited studies** — the paper's ~150 references point to original studies, many of which include their own COD-removal / methane-yield tables — **hard** (manual scraping required) — **close** match (this is the natural way to expand the ~100 rows in this paper to ~500+ rows for a more credible ML model).
  - **OpenAlex / Semantic Scholar API** — to find more recent (post-2014) anaerobic-digestion review papers and primary studies that would update the dataset — **easy** API access — **analogy** (data-discovery tool, not a benchmark).
- **Data access difficulty (for the paper's own dataset):** **Medium** — Tables 1, 2, 5 must be transcribed by hand from the PDF (no structured data download); ranges (e.g., "50–70%") need a decision rule (midpoint vs. interval). For a richer dataset, the cited references would need to be scraped — **hard**.
- **Tooling I would need to learn:** **Already in current toolset** — pandas, scikit-learn, XGBoost / LightGBM, SHAP for feature importance. Optional: `tabula-py` or `camelot` for PDF table extraction (one new but lightweight piece). No domain-specific chemistry tooling required.
- **Portfolio angle — what a good GitHub project based on this would look like:** **"ML-guided wastewater anaerobic-digestion screening for pulp and paper mills."** The project would (i) hand-transcribe Tables 1, 2, 5 of Meyer & Edwards (2014) into a CSV with ~80 rows × ~15 columns (wastewater type, COD, inhibitors, reactor type, COD removal rate, methane yield), (ii) train a gradient-boosted regressor with leave-one-out CV (small dataset → conservative validation), (iii) report feature importances via SHAP — likely surfacing sulfide concentration, AOX, and resin acids as top negative predictors, COD concentration and reactor type as top positive predictors, (iv) build a small Streamlit / Gradio demo where a mill operator inputs their stream's COD + inhibitor profile and gets a predicted COD-removal range with uncertainty bounds + flagged primary inhibitors of concern, (v) optionally seed a "v2" by scraping the paper's 150 references to expand the dataset. README headline: *"Predicting anaerobic-digestion performance for pulp & paper mill effluents from feedstock composition — an ML-on-tabular-review-data case study with feature attribution for inhibitor risk assessment."* This is a **textbook tabular-ML portfolio piece** with a sustainability framing, and it's much more legible to recruiters than the protein-language-model project for Article 1 (though smaller in dataset terms).

#### Ranking for Article 2

| Variable | Score (1–10) | One-line justification |
|---|---|---|
| Data availability | 6 | Tables 1, 2, 5 of the paper are themselves a hand-curatable ~80-row tabular dataset directly extractable from the PDF; no public benchmark dataset exists for this domain, but the paper's references cite ~150 primary studies for dataset expansion — **loose-to-close** match for the tabular-regression reframing. |
| Data access ease | 4 | Manual PDF table transcription (or `tabula-py` / `camelot` extraction) is required; ranges in cells (e.g., "50–70%") need a decision rule. The cited references would each need to be fetched and parsed for dataset expansion — **scraping needed**. |
| Task tractability on laptop / free GPU | 9 | ~80–100 rows of tabular data; XGBoost / LightGBM trains in **seconds on CPU**; no GPU needed. **Trivially fits laptop compute.** |
| Tooling alignment with my current skills | 9 | pandas + sklearn + XGBoost + SHAP — **entirely in current toolset**. Optional `tabula-py` / `camelot` for PDF extraction is a lightweight new piece. **Best tooling alignment of any Edwards-lab paper so far.** |
| Problem clarity | 7 | COD removal rate (%) and specific methane yield (m³ kg⁻¹ COD removed) are **clean numerical regression targets** with standard units. The main ambiguity is that many table cells are reported as ranges rather than point estimates — but this is a routine modeling decision (midpoint, interval regression, or quantile regression). **Cleanest problem framing of the Edwards papers so far.** |
| Reproducibility signals | 4 | **No code released** (it's a Review). Tables in PDF, not as a structured data file. Reported metrics (COD removal %, methane yield) are standard. **Metric framing is reproducible; the data extraction is the user's burden.** |
| Portfolio impact | 5 | Tabular ML with ~100 rows is **unimpressive on its own** to recruiters who weight model novelty / scale. Sustainability / wastewater-engineering framing is **legible but niche** — bioenergy and circular-economy themes are increasingly recruited for, but this isn't a hot subfield like LLMs / drug discovery. SHAP feature attribution + Streamlit demo gives narrative shape. **Solid mid-tier portfolio piece.** |
| My personal interest | (ask me) | (provided by student) |

**Overall score computation:**
- Provisional 7-variable mean: `(6 + 4 + 9 + 9 + 7 + 4 + 5) / 7 = 44 / 7 ≈ 6.3` (provisional — personal interest pending).

**Overall score:** **6.3 / 10** (provisional)

**One-sentence verdict:** Yes — this is a **higher-quality portfolio bet than Article 1** because the ML reframing (tabular regression on Tables 1, 2, 5) sits squarely in current ML toolset, the regression targets are clean and well-documented, and Edwards's senior authorship makes the lab signal stronger; the small dataset (~80 rows) is the main constraint and is partially addressable via reference-list scraping.

### Article 3: Biochemical and Structural Insights into Enzymatic Depolymerization of Polylactic Acid and Other Polyesters by Microbial Carboxylesterases

- **Full citation:** Hajighasemi, M., Nocek, B. P., Tchigvintsev, A., Brown, G., Flick, R., Xu, X., Cui, H., Hai, T., Joachimiak, A., Golyshin, P. N., Savchenko, A., **Edwards, E. A.\***, & Yakunin, A. F.\* (2016). Biochemical and structural insights into enzymatic depolymerization of polylactic acid and other polyesters by microbial carboxylesterases. *Biomacromolecules*, 17(6), 2027–2039. DOI: 10.1021/acs.biomac.6b00223
- **Authorship caveat:** **Edwards is 10th of 12 authors and CO-CORRESPONDING author** (her email + Yakunin's email are both listed; Yakunin is last author and likely the primary PI for the structural-biology side of the work). Mahbod Hajighasemi (1st author) is the lead bench scientist. **This is the strongest authorship signal of the three Edwards papers reviewed so far** — co-corresponding-author position on a **primary research paper** (not a Review) indicates Edwards's group co-led this study and has direct stake in the methodology. The paper was supported by Genome Canada via the same NSERC / OGI 2009-OGI-ABC-1405 grant cited in Article 1, suggesting **a third Edwards-lab research direction** (enzyme discovery for plastic biodegradation), distinct from the chlorinated-solvent bioremediation (Article 1) and pulp/paper wastewater bioenergy (Article 2) directions.
- **Problem in plain English:** **Polylactic acid (PLA) is a bioplastic** made from renewable feedstocks (corn starch → lactic acid → PLA polymerization) that's marketed as a compostable replacement for petroleum-based plastics. Global PLA production is growing ~40% per year and now occupies >20% of the bioplastics market. The catch: PLA is *industrially compostable* (high-temperature, controlled conditions) but degrades poorly under ambient conditions, and existing recycling routes break it down to CO₂ rather than recovering the lactic acid monomer. **Enzymatic depolymerization** could recover lactic acid for repolymerization (closed-loop recycling), but only a few PLA-degrading enzymes are known and there's no crystal structure to guide enzyme engineering. The paper screens **90 purified microbial α/β-hydrolases** for activity against emulsified PLA, identifies two new PLA-active enzymes (ABO2449 from *Alcanivorax borkumensis* and RPA1511 from *Rhodopseudomonas palustris*), determines the **first crystal structure of a PLA depolymerase** (RPA1511 at 2.2 Å, PDB 4PSU), and uses site-directed mutagenesis (alanine scanning of active-site residues) to identify residues essential for PLA hydrolysis but **not** required for activity against simple monoester substrates — a structural motif (Trp218 + neighboring hydrophobic residues) that could be used to find more PLA depolymerases by sequence search.
- **ML task type (REFRAMED — paper itself contains no ML):** Multiple complementary reframings: **(i) protein-function classification** — given an α/β-hydrolase amino-acid sequence, predict whether it has PLA-depolymerase activity (binary, hit/no-hit; 90-protein labeled set with 2–3 hits); **(ii) substrate-specificity multi-label classification** — given an enzyme sequence, predict which polyesters it hydrolyzes (Table 1 of the paper provides 23 polyester substrates × 3 enzymes labels = ~70 binary labels; ~80% sparsity); **(iii) mutation-effect regression** — given the wild-type RPA1511 / ABO2449 sequence + a single-residue alanine substitution, predict the fold-change in PLA-hydrolysis activity (Figure 8 of the paper provides ~25 mutants × 2 substrates = 50 measurements). Reframing (iii) is closest to the popular ML-on-deep-mutational-scanning literature (e.g., DeepSequence, ESM-IF1, MutCompute).
- **Input representation:**
  - Modality: **Protein amino-acid sequences** (~300–400 residues per α/β-hydrolase) and **3D structural data** (PDB 4PSU for RPA1511; homology model for ABO2449 via Phyre2).
  - Shape/size: **90 sequences** for the screening set; **264 RdhA-style** unlabelled α/β-hydrolases available via ESTHER / Pfam PF00561 / TIGR02486 for unlabelled pretraining; **PDB 4PSU** at 2.2 Å resolution; supplementary table S1 lists 86 monoester substrates with activity measurements.
  - Preprocessing in the paper: MUSCLE alignment (Clustal Omega) + MEGA6 phylogenetic tree (neighbor-joining); structure determination via SAD phasing (HKL3000 / COOT / PHENIX / REFMAC). For the ML reframing, raw sequences feed into ESM-2 directly; PDB 4PSU's coordinates feed into ESM-IF1 (inverse folding) or 3D-CNN-based featurizers.
- **Output representation:**
  - What the model predicts: **Binary (PLA-active vs. inactive) or multi-label (which polyesters)**. For the mutation-effect regression: scalar fold-change in activity vs. wild-type. For an inverse-design version: **generate** novel α/β-hydrolase sequences predicted to hydrolyze PLA with higher efficiency (similar to recent MutCompute / ESM-IF / ProteinMPNN work on PETase engineering).
  - Units / label space: Binary or [0, 23] multi-label vector for substrate prediction; log fold-change scalar for mutation effects.
- **Model architecture (paper has none — this is a suggestion):** ESM-2 (8M / 35M / 150M parameters depending on Colab tier) for protein-sequence embeddings → small MLP / logistic-regression head for substrate classification. For mutation-effect prediction: ESM-2 zero-shot scoring (no training needed for a baseline) → fine-tune on the ~50 mutation measurements with weight regularization. For inverse design: ProteinMPNN backbone-conditioned sequence design + ESM-2 substrate-classifier guidance.
- **Dataset used in the paper:**
  - Name: **Curated set of 90 purified α/β-hydrolases** (no formal database name; constructed by the Yakunin / Edwards / Savchenko consortium from sequenced microbial genomes); **86-monoester substrate library** (supplementary table S1); **PDB 4PSU** crystal structure.
  - Size: 90 enzymes × ~110 substrates (23 polyester + 86 monoester − some overlap) × hit/no-hit. The paper's reported hit set is small (2 PLA-active enzymes + 1 positive control PlaM4).
  - Source: Constructed by the authors; sequences from NCBI (accessions YP_694169 for ABO2449, NP_946859 for RPA1511); PDB 4PSU deposited by the authors at the Argonne National Lab Structural Biology Center.
  - Public? **Yes — partially.** PDB 4PSU and individual NCBI sequences are public; the full 90-protein screening list with hit/no-hit labels would need to be hand-extracted from the paper's supplementary material (supporting information PDF).
  - Link: PDB structure → https://www.rcsb.org/structure/4PSU ; sequences → https://www.ncbi.nlm.nih.gov/protein/YP_694169 and https://www.ncbi.nlm.nih.gov/protein/NP_946859 ; supplementary information → https://pubs.acs.org/doi/10.1021/acs.biomac.6b00223
- **Reported headline metric:** No predictive metric (it's a discovery + structural-biology paper). Headline experimental results: ABO2449 catalyzed **>90% conversion** of solid PLA10 to lactic acid within 36 h (with 0.1% Plysurf A210G detergent); RPA1511 reached ~40% PLA conversion in the same window. Crystal structure refined to R_work / R_free = 0.181 / 0.221 at 2.2 Å. Site-directed mutagenesis identified **Thr48, Gln172, Met215, Trp218, Leu220** in RPA1511 (and **Phe38, Leu152** in ABO2449) as residues essential for PLA hydrolysis but **not** for monoester hydrolysis — a candidate structural motif for sequence-based search of new PLA depolymerases.
- **Public alternatives to replicate or approximate this:**
  - **ESTHER database of α/β-hydrolases** (cited in the paper) — ~50,000 sequences with functional annotations — **easy** access (HTTP download) — https://bioweb.supagro.inra.fr/ESTHER/general — **close** match for the protein-function-classification reframing.
  - **Pfam PF00561** (α/β-hydrolase fold) and **TIGR02486 / Pfam PF13486** (reductive dehalogenase, overlap with Article 1) — protein family databases via InterPro — **easy** — https://www.ebi.ac.uk/interpro/entry/pfam/PF00561/ — **close** match.
  - **PETase / FAST-PETase / leaf-compost cutinase** datasets — recent (2022–2024) high-profile plastic-degrading-enzyme engineering papers (Tournier et al. *Nature* 2020; Lu et al. *Nature* 2022) released sequence and activity data — **medium** access (per-paper supplementary) — **close** match (PET is a different polyester from PLA but the engineering framework is identical) — these are the **portfolio benchmark** to compare against.
  - **PDB 4PSU + homologous structures via Dali / FoldSeek** — structural search returns hundreds of α/β-hydrolases with crystal structures — **easy** — https://search.foldseek.com/ — **close** match for structure-based featurizers.
  - **ProteinMPNN / ESM-IF1 / RFdiffusion** — pretrained inverse-folding / generative models for protein design — **easy** (HuggingFace + GitHub) — **analogy** match (tooling, not data).
- **Data access difficulty (for the paper's own dataset):** **Medium** — PDB structure and individual sequences are one-click downloads, but the 90-protein screening list with hit/no-hit labels and the 86-monoester activity matrix need to be transcribed from the supplementary information PDF. The mutation-effect data (Figure 8) needs to be digitized from the bar charts (use `WebPlotDigitizer` or read off Table values).
- **Tooling I would need to learn:** **BioPython** (already in toolset for Article 1), **HuggingFace `transformers` + ESM-2** (already flagged for Article 1), **PyMOL / Biopython structure parsing** for PDB 4PSU (lightweight, mostly already in toolset for any chemistry-adjacent project), **ProteinMPNN / ESM-IF1** (one new piece, but well-documented HuggingFace integrations exist), **RDKit** for the 86-monoester substrate molecules (already in toolset from Beharry / Seferos articles).
- **Portfolio angle — what a good GitHub project based on this would look like:** **"PLA depolymerase activity prediction from protein sequence — an ESM-2 + structural-motif-search pipeline grounded in PDB 4PSU."** The project would (i) collect the 90-protein screening set with hit/no-hit labels from supplementary table; (ii) compute ESM-2 embeddings for each of the 90 proteins and train a logistic-regression / small-MLP classifier (with leave-one-out CV for the small positive class — 2–3 hits of 90); (iii) **the killer feature**: define the "Trp218 motif" identified in the paper (Trp218 + Met215 + Leu212 + Leu220 spatial arrangement) as a structural fingerprint; use FoldSeek / Dali to search the entire AlphaFold Database (200M+ structures) for proteins matching this motif and rank candidates by ESM-2 classifier score for downstream wet-lab validation; (iv) extend to PETase / leaf-cutinase by including those datasets as out-of-distribution test set (the paper's Trp motif may or may not generalize); (v) optional: ProteinMPNN-based inverse design of RPA1511 variants predicted to have higher PLA-hydrolysis activity (this is the **"FAST-PETase but for PLA"** angle that has high recruiter visibility). README headline: *"Re-discovering microbial PLA depolymerases at scale — combining ESM-2 sequence embeddings with a structural Trp218 motif from PDB 4PSU to rank AlphaFoldDB candidates for plastic-recycling enzyme engineering."* This is a **directly hot-topic** portfolio piece (PETase / plastic-recycling enzyme engineering has been on Nature / Science covers in 2020 / 2022) and arguably the **best-positioned of any of the Edwards-lab papers for ML-recruiter visibility**.

#### Ranking for Article 3

| Variable | Score (1–10) | One-line justification |
|---|---|---|
| Data availability | 6 | PDB 4PSU + GenBank sequences are one-click public; the 90-protein hit/no-hit labels and 86-monoester activity matrix require supplementary-material extraction. ESTHER + Pfam α/β-hydrolase databases provide tens of thousands of sequences for unlabelled pretraining; AlphaFoldDB provides predicted structures for nearly all known α/β-hydrolases. **Loose-to-close** match overall. |
| Data access ease | 5 | One-click for the structure and individual sequences; manual transcription from supplementary PDF for the screening labels and monoester matrix; bar-chart digitization for the mutation-effect data. **Scripting + manual extraction needed**. |
| Task tractability on laptop / free GPU | 9 | 90 proteins × ESM-2 embeddings fits free Colab in minutes. Structural-motif search via FoldSeek runs on CPU. AlphaFoldDB-scale screening is feasible if scoped to a small subfamily (e.g., 10K α/β-hydrolases). **Free-tier compute is sufficient**. |
| Tooling alignment with my current skills | 5 | Same as Article 1 (BioPython + ESM-2 + sklearn) plus light additions (Biopython structure parsing, FoldSeek, optionally ProteinMPNN). **One major new piece** (protein-language models / inverse-folding), but the overlap with Article 1's tooling means most of the learning is amortized across both projects. |
| Problem clarity | 5 | Hit/no-hit binary classification is clean **but the labelled set is tiny** (2–3 of 90 positives — extreme class imbalance). Substrate-specificity multi-label is more forgiving. Mutation-effect regression has ~50 measurements — small but tractable. The structural-motif feature engineering is well-documented in the paper. **Clear framing, small data**. |
| Reproducibility signals | 5 | **PDB structure deposited (4PSU)**, **GenBank accessions provided**, supplementary tables included. **No code released** (it's a primary biology paper). Methods section is detailed (HKL3000, PHENIX, REFMAC parameters specified). **Better reproducibility than Articles 1 or 2** thanks to the PDB deposition. |
| Portfolio impact | 7 | **Plastic recycling / bioplastic depolymerization is a HOT sustainability topic** — PETase, FAST-PETase, and leaf-compost cutinase have been on *Nature* and *Science* covers in 2020 and 2022. PLA depolymerization is directly adjacent. A "Trp motif + ESM-2 + AlphaFoldDB screen" project sits **squarely in the protein-engineering-for-sustainability narrative** that is currently well-funded and well-recruited-for. **Highest portfolio-impact score of any Edwards-lab paper** and arguably competitive with the best Sánchez-Lengeling / Moosavi articles. |
| My personal interest | (ask me) | (provided by student) |

**Overall score computation:**
- Provisional 7-variable mean: `(6 + 5 + 9 + 5 + 5 + 5 + 7) / 7 = 42 / 7 = 6.0` (provisional — personal interest pending).

**Overall score:** **6.0 / 10** (provisional)

**One-sentence verdict:** Yes — this is the **most portfolio-relevant of the three Edwards-lab papers** because (a) it's a primary research paper with Edwards as co-corresponding author (strongest lab signal), (b) the topic (enzymatic plastic recycling) is currently a hot ML-for-sustainability subfield, and (c) the PDB 4PSU structure + identified Trp218 motif provide a clean **"sequence + structure → substrate specificity"** project framing that pairs naturally with ESM-2 + FoldSeek + AlphaFoldDB screening; the ~90-protein labelled set is the main constraint and is partially addressable by including PETase / cutinase data from independent papers.

#### Professor-level rollup

- **Article 1 overall:** 5.4 (provisional)
- **Article 2 overall:** 6.3 (provisional)
- **Article 3 overall:** 6.0 (provisional)
- **Professor overall** = (5.4 + 6.3 + 6.0) / 3 = 17.7 / 3 = 5.9 ≈ **5.9 / 10** (provisional — all three articles awaiting personal-interest scores)

### Summary for this professor

- **Common thread across their work (BASED ON A SINGLE REVIEW PAPER — INFERRED):** Anaerobic microbial bioremediation of chlorinated groundwater pollutants via organohalide-respiring bacteria; emphasis on metagenomics of enrichment cultures (KB-1, ACT-3, WBC-2) and characterization of reductive dehalogenase enzyme families. **Re-evaluate after additional Edwards-lab first-author / corresponding-author papers are added.**
- **Dominant data modality (inferred from this paper alone):** Protein amino-acid sequences (RdhA family, curated set of 264 from 44 organisms), with metagenomics / 16S rRNA gene sequence data as adjacent modality from her enrichment-culture work cited in the Review.
- **Biggest obstacle for me to replicate their work publicly:** **Label scarcity** — only ~15 of the 264 reductive dehalogenases have biochemically characterized substrate specificity, so any supervised ML reframing has a tiny labelled set. Compute is not the bottleneck; data labels are.
- **Skills I would gain by working with them:** Anaerobic microbial ecology; metagenomics pipelines (assembly, binning, gene calling); enrichment-culture experimental design; protein-family classification and phylogenetics; environmental engineering context for ML projects (bioremediation site monitoring); BioPython / NCBI Entrez programmatic data access; protein language models (ESM-2 / ProtBERT) applied to environmental enzyme families.

### Open items / notes

- Lab webpage / Google Scholar URL for Prof. Edwards not yet confirmed — verify before finalizing.
- Personal interest score for Article 1 not yet provided by student.
- **Only one article processed so far** — single-paper professor evaluation is unstable; recommend processing 1–2 additional Edwards-lab papers (preferably first-authored or corresponding-authored by Edwards) before treating the 5.4 professor score as load-bearing for thesis-advisor selection.
- The paper's referenced Google Drive RD_OG spreadsheet (docs.google.com/folder/d/0BwCzK8wzlz8ON1o2Z3FTbHFPYXc/edit) is from 2013 and likely dead — verify availability or fall back on the supplementary materials hosted at http://dx.doi.org/10.1098/rstb.2012.0322 .
- Article 1 is a Review/Perspective paper, not a primary research paper — the ML reframing is an extrapolation of the paper's open question (sequence similarity ≠ substrate specificity), not a replication of any modelling done in the paper itself. **Be explicit about this when citing the project.**
- Authorship: Edwards is **6th of 7 (middle author)** on Article 1; her PhD student Laura A. Hug is first author. For an accurate signal of Edwards's independent research direction, future articles should prioritize first-/corresponding-authored papers from her Toronto group.
- Personal interest score for Article 2 not yet provided by student.
- Article 2 is also a Review (Meyer & Edwards 2014, *Water Research*); the ML reframing is tabular regression on Tables 1, 2, 5 — not replication of any modelling done in the paper itself. **Be explicit about this when citing the project.**
- Edwards's Article 2 portfolio is **two distinct sub-areas**: (a) chlorinated-solvent bioremediation / organohalide-respiring bacteria (Article 1 territory — KB-1, ACT-3, WBC-2 enrichment cultures, RdhA enzymes) and (b) industrial wastewater bioenergy / anaerobic digestion of pulp & paper mill streams (Article 2 territory). The Summary section above (which was written when only Article 1 was processed) characterizes the lab as bioremediation-focused and **is now stale** — re-evaluate "Common thread" / "Dominant data modality" wording after a 3rd article is processed, since the lab clearly straddles both directions and a single dominant modality may not be appropriate.
- Authorship contrast: Edwards is **last/senior author of 2** on Article 2 (much stronger lab-signal than Article 1's 6-of-7 position). The professor-level score moved from 5.4 → 5.9 mostly because Article 2's tabular-ML angle is more tractable (data access + tooling alignment + problem clarity all scored 7+); the Edwards-lab signal is now **provisionally credible** for the wastewater-engineering side of her portfolio.
- Dataset extraction: Article 2's Tables 1, 2, 5 transcribe to ~80 rows total — small for credible ML. Document the dataset-curation step (PDF → CSV) as **the first half** of the GitHub project and treat it as a deliverable in its own right.
- Personal interest score for Article 3 not yet provided by student.
- Article 3 is a **primary research paper** (Hajighasemi et al. 2016, *Biomacromolecules*) — in contrast to Articles 1 and 2 (both Reviews). Edwards is **co-corresponding author** (10th of 12) — the strongest authorship signal of the three papers. **The lab clearly engages in primary methodology work, not just reviewing.**
- Edwards's lab **spans three distinct research directions**, not two as previously inferred: (a) chlorinated-solvent bioremediation / organohalide-respiring bacteria (Article 1), (b) industrial wastewater bioenergy / anaerobic digestion of pulp & paper mill streams (Article 2), and (c) enzyme discovery for plastic depolymerization / bioplastic recycling (Article 3). The Summary section above (written when only Article 1 existed) is **doubly stale** now — it characterizes the lab as bioremediation-focused, but Articles 2 and 3 are not bioremediation. **Recommend rewriting the Summary section explicitly** before treating it as authoritative; flagging here rather than rewriting because per the saved scoping rule the user prefers additive edits unless rewrites are explicitly approved. **User: please confirm whether to rewrite the Summary section to reflect all three research directions.**
- The professor-level score is unchanged at **5.9 / 10** (Article 3 at 6.0 sits exactly at the running mean), but the **lab-signal interpretation is materially different** with Article 3 added: Edwards is now confirmed as co-corresponding-author on a primary research paper, which significantly raises the credibility of the lab's flagship work compared to a 2-Review evaluation.
- Article 3's PDB structure 4PSU (https://www.rcsb.org/structure/4PSU) is publicly deposited — a **higher reproducibility signal than Articles 1 or 2** (neither of which has structural data).
- Plastic-recycling enzymes (PETase, FAST-PETase, leaf-compost cutinase) have been on *Nature* and *Science* covers in 2020 and 2022 — Article 3 sits in this subfield and is **the most portfolio-visible of the three Edwards papers** for ML-recruiter purposes. If forced to pick one Edwards-lab GitHub project, **Article 3 is the best bet** despite being numerically tied with Article 2 on score.
- Authorship chain across the three articles: Article 1 (middle author, 6/7) → Article 2 (senior author, 2/2) → Article 3 (co-corresponding, 10/12). The trend shows **Edwards's group is increasingly central to the work**, which is reassuring for thesis-advisor selection. **Pre-2013 papers from before her independent group ramped up may have weaker signal — check Edwards's Google Scholar citation trajectory before reading older papers.**
